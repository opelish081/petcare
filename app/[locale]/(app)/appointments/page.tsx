// app/[locale]/(app)/appointments/page.tsx — หน้านัดหมายทั้งหมด
      ...
      <section>
        ...
        {upcoming.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
            ...
            <Link
              href={`/${locale}/appointments/new`}
              className="inline-flex items-center gap-2 mt-4 bg-green-500 hover:bg-green-600 text-white text-sm font-medium px-4 py-2 rounded-xl transition-colors"
            >
              <Plus size={16} />
              {t('newAppointment')}
            </Link>
...
function AppointmentCard({
  ...
  return (
    <Link
      ...
      className="block bg-white rounded-2xl border border-gray-100 p-4 hover:border-green-200 hover:shadow-sm transition-all"
    >
      ...
      {appt.location && (
        <p className="text-xs text-gray-400 mt-2 ml-14">📍 {appt.location}</p>
      )}
    </Link>
  )
}
